...
//! - Execution cannot escape the declared access set — `nex-state` rejects
...
use nex_state::{Receipt, State};
use nex_types::{AccountId, Hash, Transaction};
...
fn touchable_accounts(tx: &Transaction) -> BTreeSet<AccountId> {
    ...
    match &tx.kind {
        nex_types::TransactionKind::Transfer { to, .. }
        | nex_types::TransactionKind::Mint { to, .. } => {
            ids.insert(*to);
        }
    }
    ids
