...
fn node_hash(left: &Hash, right: &Hash) -> Hash {
    Hash::of_tagged(b"phi:smt:node", &[left.as_bytes(), right.as_bytes()])
}

fn leaf_hash(key: &Key, value_hash: &Hash) -> Hash {
    Hash::of_tagged(b"phi:smt:leaf", &[key, value_hash.as_bytes()])
}
...
fn empty(depth: usize) -> Hash {
    ...
    EMPTY.get_or_init(|| {
        ...
        hashes[DEPTH] = Hash::of_tagged(b"phi:smt:empty", &[]);
